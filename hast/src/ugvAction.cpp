...
class moveBaseActionClass
{
	private:
		...
		ros::Subscriber ugvGoal_sub, ugvRviz_sub, HastShutDown_sub;
		std::string s_ugv_goaltopic, s_map, s_kill_topic;
		std::string s_rviz_ugv_goaltopic;
	...
	public:
		moveBaseActionClass(std::string s_move_base_namespace) : ac(s_move_base_namespace, true)
		{
			...
			ros::param::get("~rviz_ugv_goaltopic", s_rviz_ugv_goaltopic);

			ugvGoal_sub 			= n.subscribe(s_ugv_goaltopic,1, 			&moveBaseActionClass::setGoal, 			this); ROS_INFO("ugvAction::subscribed to %s", s_ugv_goaltopic.c_str());
			ugvRviz_sub 			= n.subscribe(s_rviz_ugv_goaltopic,1, &moveBaseActionClass::rvizGoal, 		this); ROS_INFO("ugvAction::subscribed to %s", s_rviz_ugv_goaltopic.c_str());
			HastShutDown_sub 	= n.subscribe(s_kill_topic,1, 				&moveBaseActionClass::nodeShutDown, this); ROS_INFO("ugvAction::subscribed to %s", s_kill_topic.c_str());
		...
		void rvizGoal(const geometry_msgs::PoseStamped::ConstPtr& goalPose)
		{
