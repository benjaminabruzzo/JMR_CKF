...
%             disp(str_call);
    ...
    try data.april = data.uavSLAM.april; catch disp('  issue converting data.uavSLAM.april to data.april'); end
...
%%
    SimpleCells = {...
            ...
            'data.ugv1Recorder.oneckf.ugv_Qdk'; ...
...
%% synchronize ros times
    try
        ...
        timers.SimpleCells = {...
            ...
            'data.ugv1Recorder.est.time';...
            'data.ugv1Recorder.stereo.time';...
            'data.ugv1Recorder.veltwist.time';...
            'data.ugv1Recorder.wheel.time';...
...
%     try ugv1Recorder = data.ugv1Recorder;catch; end
...
%         };
    ...
    %     ugv1Recorder.stereo.time, ugv1Recorder.stereo.Position_gl(:,1), ...
...
% I think this code is used to generate one of the vecotr used to compute offset correlations
    try 
        x = data.ugv1Recorder.wheel.quat(:,1);
        y = data.ugv1Recorder.wheel.quat(:,2);
        z = data.ugv1Recorder.wheel.quat(:,3);
        w = data.ugv1Recorder.wheel.quat(:,4);
        data.ugv1Recorder.wheel.yaw = -atan2(2*(x.*y - z.*w ) , 1 - 2*(y.^2 + z.^2)); %atan2(2,1);
        clear x y z w
    catch
        disp("catch:: x = data.ugv1Recorder.wheel.quat(:,1);")
    end

% set vicon time using vicon.offset
    [vicon.syncoffset] = viconTimerSync(data.ugv1Recorder, vicon, eval(sprintf('data.%s',meta.ugv_leader_stereo)), data.april, data.uavRecorder);
...
%     vicon = viconsplines(vicon, ugvStereo, uavRecorder, ugv1Recorder, meta);
...
function [out] = viconTimerSync(ugv1Recorder, vicon, ugvStereo, april, uavRecorder)
% [out] = viconTimerSync(data.ugv1Recorder, data.vicon, data.ugvStereo, data.april)
    ...
    try
        [offset.dt(i,1), offset.mindiff(i,1)] = fitViconClockOffset(...
                ugv1Recorder.stereo.time, ugv1Recorder.stereo.Position_gl(:,2), ...
...
%         [offset.dt(i,1), offset.mindiff(i,1)] = fitViconClockOffset(...
%                 ugv1Recorder.est.time, ugv1Recorder.est.Yaw, ...
...
%% Spline the ugv positions of uav markers
    ...
    spline = csapi(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).time,vicon.(matlab.lang.makeValidName(meta.ugv_leader)).yaw.global);
        vector_newtime = fnval(spline, data.ugv1Recorder.stereo.time);
...
if (strcmp('20170914/', meta.date))
    ...
    %Vicon data in ugv1Recorder time
    vicon.ugvc.splines.P.global.x_ugvcesttime = fnval(vicon.ugvc.splines.P.global.x, data.ugv1Recorder.est.time);
    vicon.ugvc.splines.P.global.y_ugvcesttime = fnval(vicon.ugvc.splines.P.global.y, data.ugv1Recorder.est.time);
    vicon.ugvc.splines.P.global.z_ugvcesttime = fnval(vicon.ugvc.splines.P.global.z, data.ugv1Recorder.est.time);
    
    % Vicon position - est position
    vicon.ugvc.splines.P.global.x_diff = vicon.ugvc.splines.P.global.x_ugvcesttime - data.ugv1Recorder.est.Position_gl(:,1);
    vicon.ugvc.splines.P.global.y_diff = vicon.ugvc.splines.P.global.y_ugvcesttime - data.ugv1Recorder.est.Position_gl(:,2);
    vicon.ugvc.splines.P.global.z_diff = vicon.ugvc.splines.P.global.z_ugvcesttime - data.ugv1Recorder.est.Position_gl(:,3);

    % yaw
    vicon.ugvc.splines.yaw.est = csapi(vicon.ugvc.time,vicon.ugvc.yaw.global); 
    vicon.ugvc.splines.yaw.yaw_ugvcesttime = fnval(vicon.ugvc.splines.yaw.est, data.ugv1Recorder.est.time);
    vicon.ugvc.splines.yaw.est_err = vicon.ugvc.splines.yaw.yaw_ugvcesttime*180/pi - data.ugv1Recorder.est.Yaw;
else
    ...
    %Vicon data in ugv1Recorder time
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x, data.ugv1Recorder.est.time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y, data.ugv1Recorder.est.time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z, data.ugv1Recorder.est.time);
    ...
    % Vicon position - est position
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_diff = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_ugvkesttime - data.ugv1Recorder.est.Position_gl(:,1);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_diff = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_ugvkesttime - data.ugv1Recorder.est.Position_gl(:,2);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_diff = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_ugvkesttime - data.ugv1Recorder.est.Position_gl(:,3);
    ...
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.yaw_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.est, data.ugv1Recorder.est.time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.est_err = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.yaw_ugvkesttime*180/pi - data.ugv1Recorder.est.Yaw;
...
%     vicon.diff_yaw.splineyaw = fnval(vicon.diff_yaw.spline, ugv1Recorder.stereo.time);
%     vicon.diff_yaw.spline_error = ugv1Recorder.stereo.yaw_ugv - vicon.diff_yaw.splineyaw;
