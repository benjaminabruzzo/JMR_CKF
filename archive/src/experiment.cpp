...
class uavClass
{
 ...
 // 0.75*180/Pi; -0.75*180/Pi;
		...
		void readState(const hast::uavstate::ConstPtr& uavState_msg)
		{
			...
			// ROS_INFO("experiment: uav.readState::EstPosition_gl: [%6.4f %6.4f %6.4f]", EstPosition_gl.at<double>(0,0), EstPosition_gl.at<double>(1,0), EstPosition_gl.at<double>(2,0));
		}
		...
		void callState()
		{
			...
			ROS_WARN("callState() : %u", navStateUint);
...
class hastExperiment
{
	private:
		...
		std::FILE * pFile;
		std::string s_ugv_basefootprint, s_drive_service;
		std::string s_ugvCmd_topic, s_uav_pose_state_topic;
		...
		/*----- experiment types */
		bool pull_exp, abcde_exp, action_exp, local_exp, cmdtilt;
		bool iflift, step, slamtest;
		int phase;
	...
	hastExperiment()
	{
		...
		/*----- UAV Channels */
			...
			ros::param::get("~uav_pose_state_topic",s_uav_pose_state_topic); ROS_INFO("  experiment::s_uav_pose_state_topic  = %s", s_uav_pose_state_topic.c_str());
			uav.state_sub 		= n.subscribe(s_uav_pose_state_topic, 1, &uavClass::readState, &uav);
	...
	void experimentSlamTest()
	{
		...
		ROS_WARN("UAVstepTo((cv::Mat_<double>(3,1) << 1.75, 0.00, 0.85), 0.00, 3*uav.flytime); ");
		UAVstepTo((cv::Mat_<double>(3,1) << 1.75, 0.00, 0.85), 0.00, 3*uav.flytime); 

		ROS_WARN("slamswitch(); ...");
		slamSwitch();
		ROS_WARN("slamswitch() complete.");
	...
	void slamSwitch()
	{
		...
		fprintf (pFile,"experiment.slam.uav.EstPosition_gl(1,:) = [%6.10f %6.10f %6.10f];\n", 
			uav.EstPosition_gl.at<double>(0, 0), 
			uav.EstPosition_gl.at<double>(1, 0), 
			uav.EstPosition_gl.at<double>(2, 0));
		fprintf (pFile,"experiment.slam.uav.EstYaw_gl(1,:) = %6.10f;\n", uav.EstYaw_gl);
